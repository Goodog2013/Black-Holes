...
// WebXR (VR-шлем): асимметричный фрустум глаза — тангенсы (left, right, bottom, top)
uniform int   uXR;
uniform vec4  uXRTan;
...
void main(){
  vec3 rd;
  if(uXR > 0){
    // VR: луч через асимметричный фрустум глаза (проекция из XRView)
    vec2 t01 = gl_FragCoord.xy / uRes;
    rd = normalize(uCamMat * vec3(mix(uXRTan.x, uXRTan.y, t01.x),
                                  mix(uXRTan.z, uXRTan.w, t01.y), 1.));
  } else {
    vec2 uv = (gl_FragCoord.xy * 2. - uRes) / uRes.y;
    rd = normalize(uCamMat * vec3(uv * tan(uFov * 0.5), 1.));
  }
...
uniform int   uXR;       // VR: проекция асимметричным фрустумом
uniform vec4  uXRTan;    // тангенсы (left, right, bottom, top)
out vec3 vCol;
out vec3 vWorld;
out float vFade;

vec2 projNdc(vec3 pc, float tf){
  if(uXR > 0){
    return vec2(((pc.x / pc.z) - uXRTan.x) / (uXRTan.y - uXRTan.x),
                ((pc.y / pc.z) - uXRTan.z) / (uXRTan.w - uXRTan.z)) * 2. - 1.;
  }
  return vec2(pc.x / (pc.z * tf) * (uRes.y / uRes.x), pc.y / (pc.z * tf));
}
...
void main(){
  ...
  float tf = uXR > 0 ? (uXRTan.w - uXRTan.z) * 0.5 : tan(uFov * 0.5);
  vec2 ndc = projNdc(pc, tf);
...
uniform vec4 uVp;        // вьюпорт вывода (x, y, w, h) — в VR у каждого глаза свой
uniform int  uXR;        // VR: без виньетки и визора — оптика шлема сама «визор»
...
void main(){
  vec2 uv0 = (gl_FragCoord.xy - uVp.xy) / uVp.zw;
  vec2 uv = uv0;
  float engMask = 1.;
  if(uEngulf > 0.001){
    ...
    float rr = length((uv0 - 0.5) * vec2(uVp.z / uVp.w, 1.));
  ...
  // в VR не нужна: у каждого глаза свой кадр, затемнение краёв даёт сам шлем
  if(uXR == 0){
    vec2 q = uv0 * 2. - 1.;
...
void main(){
  ...
  if(uXR > 0){
    gl_Position = vec4(
      (((pc.x / pc.z) - uXRTan.x) / (uXRTan.y - uXRTan.x)) * 2. - 1.,
      (((pc.y / pc.z) - uXRTan.z) / (uXRTan.w - uXRTan.z)) * 2. - 1., 0., 1.);
    return;
  }
